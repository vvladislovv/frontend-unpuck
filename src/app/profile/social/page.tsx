...
export default function SocialPage() {
  ...
  const loadSocialLinks = async () => {
    try {
      ...
      toast.error('Не удалось загрузить социальные сети. Показаны демонстрационные данные.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const loadUserData = async (platform: string, username: string) => {
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        ...
        setUserData(null)
        toast.error(response.data.message || 'Не удалось загрузить данные пользователя')
      }
    } catch (error: any) {
  ...
  const handleAddLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || 'Ошибка добавления социальной сети')
      }
    } catch (error: any) {
      console.error('Ошибка добавления социальной сети:', error)
      toast.error('Не удалось добавить социальную сеть')
    }
  }
  ...
  const handleRemoveLink = async (linkId: string) => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(response.data.message || 'Ошибка удаления социальной сети')
      }
    } catch (error: any) {
        ...
        <div className="px-4 py-6 space-y-6">
          ...
          <div className="space-y-4">
            {Array.isArray(socialLinks) && socialLinks.map((link, index) => (
              <div key={link.id || `link-${index}`} className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <span className="text-2xl">{getPlatformIcon(link.platform)}</span>
                    <div>
                      <div className="flex items-center space-x-2">
                        <h3 className="font-medium text-gray-900">{getPlatformLabel(link.platform)}</h3>
                        {link.verified && (
                          <span className="inline-block w-4 h-4 bg-green-500 rounded-full flex items-center justify-center">
          ...
          {!showAddForm ? (
            ...
            <form onSubmit={handleAddLink} className="bg-white rounded-lg p-4 shadow-sm border border-gray-200 space-y-4">
              <h3 className="text-lg font-medium text-gray-900">Добавить социальную сеть</h3>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    setNewLink(prev => ({ ...prev, platform: e.target.value as any, username: '', url: '' }))
                    setUserData(null)
                  }}
              ...
              <div>
                ...
                <div className="flex space-x-2">
                  <input
                    ...
                    onBlur={() => loadUserData(newLink.platform, newLink.username)}
                    required
                  ...
                  <button
                    type="button"
                    onClick={() => loadUserData(newLink.platform, newLink.username)}
              ...
              {userData && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="font-medium text-blue-900 mb-2">Данные пользователя:</h4>
                  <div className="flex items-center space-x-3">
                    {userData.avatar && (
                      <img 
                        src={userData.avatar} 
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-blue-700">{userData.username}</p>
                      {userData.followers && (
                      ...
                      {userData.bio && (
                        <p className="text-xs text-blue-600 mt-1">{userData.bio}</p>
                      )}
                    </div>
                    {userData.verified && (
